...
namespace {
    static constexpr D3D_FEATURE_LEVEL MIN_FEATURE_LEVEL
        = D3D_FEATURE_LEVEL::D3D_FEATURE_LEVEL_12_0; //最低要求機能レベル
...
namespace Framework::DX {
    ...
    DeviceResource::DeviceResource(
        ...
        if (mOptions & REQUIRE_TEARING_SUPPORT) { mOptions |= ALLOW_TEARING; }
    }
    //デストラクタ
    DeviceResource::~DeviceResource() {
        //終了前にGPUの処理を終わらせる
    ...
    void DeviceResource::initializeDXGIAdapter() {
        ...
        if (SUCCEEDED(DXGIGetDebugInterface1(0, IID_PPV_ARGS(&dxgiInfoQueue)))) {
            useDebug = true;
            MY_THROW_IF_FAILED(
                CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, IID_PPV_ARGS(&mFactory)));
            dxgiInfoQueue->SetBreakOnSeverity(DXGI_DEBUG_ALL,
                DXGI_INFO_QUEUE_MESSAGE_SEVERITY::DXGI_INFO_QUEUE_MESSAGE_SEVERITY_ERROR, true);
            dxgiInfoQueue->SetBreakOnSeverity(DXGI_DEBUG_ALL,
                DXGI_INFO_QUEUE_MESSAGE_SEVERITY::DXGI_INFO_QUEUE_MESSAGE_SEVERITY_CORRUPTION,
                true);
        }
#endif

        if (!useDebug) { MY_THROW_IF_FAILED(CreateDXGIFactory1(IID_PPV_ARGS(&mFactory))); }
    ...
    void DeviceResource::createDeviceResources() {
        ...
        if (SUCCEEDED(mDevice.As(&infoQueue))) {
            ...
            infoQueue->SetBreakOnSeverity(
                D3D12_MESSAGE_SEVERITY::D3D12_MESSAGE_SEVERITY_CORRUPTION, true);
            infoQueue->SetBreakOnSeverity(
                D3D12_MESSAGE_SEVERITY::D3D12_MESSAGE_SEVERITY_ERROR, true);
#endif
        ...
        D3D12_FEATURE_DATA_FEATURE_LEVELS featLevels = { _countof(FEATURE_LEVELS), FEATURE_LEVELS,
            D3D_FEATURE_LEVEL::D3D_FEATURE_LEVEL_11_0 };
        MY_THROW_IF_FAILED(mDevice->CheckFeatureSupport(
            D3D12_FEATURE::D3D12_FEATURE_FEATURE_LEVELS, &featLevels, sizeof(featLevels)));
        ...
        for (UINT n = 0; n < BACK_BUFFER_COUNT; n++) {
            MY_THROW_IF_FAILED(mDevice->CreateCommandAllocator(
        ...
        if (!mFenceEvent.IsValid()) { MY_THROW_IF_FAILED(E_FAIL); }
    }
    //ウィンドウに関連するリソース作成
    void DeviceResource::createWindowDependentResources() {
        ...
        if (mSwapChain) {
            HRESULT hr = mSwapChain->ResizeBuffers(BACK_BUFFER_COUNT, backBufferWidth,
                backBufferHeight, backBufferFormat,
                (mOptions & ALLOW_TEARING)
        ...
        //まだ存在しなければ新しく作成
            ...
            swapChainDesc.Flags = (mOptions & ALLOW_TEARING)
                ? DXGI_SWAP_CHAIN_FLAG::DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING
                : 0;
            ...
            if (prevIsFullScreen) { mWindow->setWindowZOrderToTopMost(false); }

            MY_THROW_IF_FAILED(mFactory->CreateSwapChainForHwnd(mCommandQueue.Get(),
                mWindow->getHWnd(), &swapChainDesc, &fullScreenChainDesc, nullptr, &swapChain));

            if (prevIsFullScreen) { mWindow->setWindowZOrderToTopMost(true); }
    ...
    bool DeviceResource::windowSizeChanged(UINT width, UINT height, bool minimized) {
        ...
        //前と同じ大きさなら終了
        if (newRect == mOutputSize) { return false; }
    ...
    void DeviceResource::handleDeviceLost() {
        //ロストしたことを通知
        if (mDeviceNotify) { mDeviceNotify->onDeviceLost(); }
        ...
        //デバイスの復活を通知
        if (mDeviceNotify) { mDeviceNotify->onDeviceRestored(); }
    }
    ...
    void DeviceResource::moveToNextFrame() {
        ...
        if (mFence->GetCompletedValue() < mFenceValues[mBackBufferIndex]) {
            MY_THROW_IF_FAILED(
                mFence->SetEventOnCompletion(mFenceValues[mBackBufferIndex], mFenceEvent.Get()));
            WaitForSingleObjectEx(mFenceEvent.Get(), INFINITE, FALSE);
        }
