...
namespace Framework::DX {
    ...
    //デストラクタ
    GPUUploadBuffer::~GPUUploadBuffer() {
        if (mResource) { mResource->Unmap(0, nullptr); }
    }
    //アロケート
    void GPUUploadBuffer::allocate(ID3D12Device* device, UINT bufferSize, LPCWSTR name) {
        ...
        MY_THROW_IF_FAILED(mResource->SetName(name));
    }
    //CPU書き込み範囲を返す
