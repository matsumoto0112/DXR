...
namespace {
    ...
    static const std::unordered_map<BottomLevelASType::MyEnum, std::wstring>
        MODEL_NAMES = {
    ...
    inline void createBuffer(ID3D12Device* device, ID3D12Resource** resource,
        void* data, UINT64 size, LPCWSTR name = nullptr) {
        CD3DX12_HEAP_PROPERTIES heapProps(
            D3D12_HEAP_TYPE::D3D12_HEAP_TYPE_UPLOAD);
        CD3DX12_RESOURCE_DESC bufferDesc = CD3DX12_RESOURCE_DESC::Buffer(size);
        MY_THROW_IF_FAILED(device->CreateCommittedResource(&heapProps,
            D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &bufferDesc,
    ...
    template <class T>
    inline std::vector<T> toLinearList(
    ...
    inline std::vector<Framework::DX::Vertex> toLinearVertices(
        ...
        for (size_t i = 0; i < positions.size(); i++) {
            for (size_t j = 0; j < positions[i].size(); j++) {
                ...
                v.tangent
                    = tangents.empty() ? Vec4(0, 0, 0, 0) : tangents[i][j];
                res.emplace_back(v);
            }
        }
        return res;
    ...
    inline void allocateUAVBuffer(ID3D12Device* device, UINT64 bufferSize,
        ...
        MY_THROW_IF_FAILED(device->CreateCommittedResource(&heapProp,
            D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &bufferDesc,
            initResourceState, nullptr, IID_PPV_ARGS(resource)));
...
Scene::Scene(Framework::DX::DeviceResource* device,
    ...
    Framework::Desc::DescriptorTableDesc desc
        = { L"ResourceTable", 10000, Framework::Desc::HeapType::CBV_SRV_UAV,
              Framework::Desc::HeapFlag::ShaderVisible };
    mDescriptorTable
        = std::make_unique<CountingDescriptorTable>(device->getDevice(), desc);
...
#define CAMERA_ROTATION_PARAMS(name, type, min, max)                         \
    ...
    mDebugWindow->addItem(
        std::make_shared<Framework::ImGUI::Text>("LightPosition"));
...
void Scene::update() {
    ...
    Mat4 view = Mat4::createRotation(mCameraRotation)
    ...
    if (mInputManager->getKeyboard()->getKey(
            Framework::Input::KeyCode::LShift)) {
        mQuadRotate += 1.0f;
    }
}
...
void Scene::render() {
    ...
    for (UINT n = 0; n < TRIANGLE_COUNT; n++) {
        ...
        XMMATRIX transform = XMMatrixRotationRollPitchYaw(
                                 rotX.getRad(), rotY.getRad(), rotZ.getRad())
        ...
        instanceDesc[n + offset].Flags = D3D12_RAYTRACING_INSTANCE_FLAGS::
        ...
        instanceDesc[n + offset].AccelerationStructure
            = mBLASBuffers[BottomLevelASType::UFO]
                  .buffer->GetGPUVirtualAddress();
        XMStoreFloat3x4(
            reinterpret_cast<XMFLOAT3X4*>(instanceDesc[n + offset].Transform),
            transform);
    }
    ...
    for (UINT n = 0; n < QUAD_COUNT; n++) {
        XMMATRIX transform = XMMatrixScaling(1, 1, 1)
            * XMMatrixRotationRollPitchYaw(
                0, mQuadRotate.toRadians().getRad(), 0)
        ...
        instanceDesc[n + offset].AccelerationStructure
            = mBLASBuffers[BottomLevelASType::Quad]
                  .buffer->GetGPUVirtualAddress();
        XMStoreFloat3x4(
            reinterpret_cast<XMFLOAT3X4*>(instanceDesc[n + offset].Transform),
            transform);
    ...
    for (UINT n = 0; n < FLOOR_COUNT; n++) {
        XMMATRIX transform
            = XMMatrixScaling(100, 1, 100) * XMMatrixTranslation(0, -5, 0);
        instanceDesc[n + offset].InstanceID = 0;
        instanceDesc[n + offset].InstanceMask = 0xff;
        instanceDesc[n + offset].Flags = D3D12_RAYTRACING_INSTANCE_FLAGS::
            D3D12_RAYTRACING_INSTANCE_FLAG_NONE;
        ...
        instanceDesc[n + offset].AccelerationStructure
            = mBLASBuffers[BottomLevelASType::Floor]
                  .buffer->GetGPUVirtualAddress();
        XMStoreFloat3x4(
            reinterpret_cast<XMFLOAT3X4*>(instanceDesc[n + offset].Transform),
            transform);
    ...
    createBuffer(device, mInstanceDescs.ReleaseAndGetAddressOf(),
        instanceDesc.data(),
        instanceDesc.size() * sizeof(D3D12_RAYTRACING_INSTANCE_DESC),
        L"InstanceDescs");
    topLevelBuildDesc.Inputs.InstanceDescs
        = mInstanceDescs->GetGPUVirtualAddress();

    dxrCommandList->BuildRaytracingAccelerationStructure(
    ...
    dispatchDesc.HitGroupTable.StartAddress
        = mHitGroupTable->GetGPUVirtualAddress();
    ...
    preCopyBarriers[0] = CD3DX12_RESOURCE_BARRIER::Transition(
        mDeviceResource->getRenderTarget(),
        D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RENDER_TARGET,
        D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_COPY_DEST);
    preCopyBarriers[1] = CD3DX12_RESOURCE_BARRIER::Transition(
        mRaytracingOutput.mResource.Get(),
...
void Scene::createDeviceDependentResources() {
    mDXRDevice->recreate();
    {
        mGpuTimer.storeDevice(mDeviceResource->getDevice(),
            mDeviceResource->getCommandQueue(),
        ...
        auto serializeAndCreateRootSignature = [&](ID3D12Device* device,
            ...
            MY_THROW_IF_FAILED_LOG(
                D3D12SerializeRootSignature(&desc,
                    ...
                    &blob, &error),
                error
                    ? toString(static_cast<wchar_t*>(error->GetBufferPointer()))
                    : "");
            MY_THROW_IF_FAILED(
                device->CreateRootSignature(1, blob->GetBufferPointer(),
                    blob->GetBufferSize(), IID_PPV_ARGS(&(*rootSig))));
        };
        {
            CD3DX12_DESCRIPTOR_RANGE ranges[3];
            ranges[0].Init(
                D3D12_DESCRIPTOR_RANGE_TYPE::D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1,
                0);
            ranges[1].Init(
                D3D12_DESCRIPTOR_RANGE_TYPE::D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1,
                1);
            ranges[2].Init(
                D3D12_DESCRIPTOR_RANGE_TYPE::D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1,
                2);
            ...
            rootParams[GlobalRootSignature::Slot::RenderTarget]
                .InitAsDescriptorTable(1, &ranges[0]);
            rootParams[GlobalRootSignature::Slot::AccelerationStructure]
                .InitAsShaderResourceView(0);
            rootParams[GlobalRootSignature::Slot::SceneConstant]
                .InitAsConstantBufferView(0);
            rootParams[GlobalRootSignature::Slot::IndexBuffer]
                .InitAsDescriptorTable(1, &ranges[1]);
            rootParams[GlobalRootSignature::Slot::VertexBuffer]
                .InitAsDescriptorTable(1, &ranges[2]);
            ...
            CD3DX12_ROOT_SIGNATURE_DESC global(
                _countof(rootParams), rootParams, _countof(sampler), sampler);
        ...
        {
            {
                ...
                CD3DX12_ROOT_SIGNATURE_DESC local(
                    _countof(rootParams), rootParams);
                local.Flags = D3D12_ROOT_SIGNATURE_FLAGS::
            ...
            }
            {
                ...
                range[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE::
                                  D3D12_DESCRIPTOR_RANGE_TYPE_SRV,
                    1, 3);
                range[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE::
                                  D3D12_DESCRIPTOR_RANGE_TYPE_SRV,
                    1, 4);
                range[2].Init(D3D12_DESCRIPTOR_RANGE_TYPE::
                                  D3D12_DESCRIPTOR_RANGE_TYPE_SRV,
                    1, 5);
                range[3].Init(D3D12_DESCRIPTOR_RANGE_TYPE::
                                  D3D12_DESCRIPTOR_RANGE_TYPE_SRV,
                    1, 6);
                ...
                params[LocalRootSignature::HitGroup::Constants::Normal]
                    .InitAsDescriptorTable(1, &range[1]);
                params[LocalRootSignature::HitGroup::Constants::MetalRough]
                    .InitAsDescriptorTable(1, &range[2]);
                params[LocalRootSignature::HitGroup::Constants::Emissive]
                    .InitAsDescriptorTable(1, &range[3]);

                params[LocalRootSignature::HitGroup::Constants::SceneConstants]
                    .InitAsConstants(contSize, 1);
                ...
                serializeAndCreateRootSignature(mDeviceResource->getDevice(),
                    ...
                    &mHitGroupLocalRootSignature
                        [LocalRootSignature::HitGroup::Normal]);
            }
        }
    }
    {
        ...
        {
            auto exportShader = [&pipeline](void* shaderFile, SIZE_T shaderSize,
                                    const auto&... names) {
                CD3DX12_DXIL_LIBRARY_SUBOBJECT* lib
                    = pipeline
                          .CreateSubobject<CD3DX12_DXIL_LIBRARY_SUBOBJECT>();
                lib->SetDXILLibrary(
                    &CD3DX12_SHADER_BYTECODE(shaderFile, shaderSize));
                for (auto&& name :
                    std::initializer_list<std::wstring>{ names... }) {
                    lib->DefineExport(name.c_str());
                }
            };

            exportShader((void*)g_pMiss, _countof(g_pMiss), MISS_SHADER_NAME);
            exportShader((void*)g_pClosestHit_Normal,
                _countof(g_pClosestHit_Normal), CLOSEST_HIT_NORMAL_NAME);
            exportShader((void*)g_pClosestHit_Plane,
                _countof(g_pClosestHit_Plane), CLOSEST_HIT_PLANE_NAME);
            exportShader((void*)g_pRayGenShader, _countof(g_pRayGenShader),
                RAY_GEN_NAME);
            exportShader(
                (void*)g_pShadow, _countof(g_pShadow), MISS_SHADOW_SHADER_NAME);
        }
        {
            auto bindHitGroup = [&pipeline](const std::wstring& hitGroupName,
                                    D3D12_HIT_GROUP_TYPE type,
                                    const std::wstring& closestHit = L"",
                                    const std::wstring& anyHit = L"",
                                    const std::wstring& intersection = L"") {
                CD3DX12_HIT_GROUP_SUBOBJECT* hitGroup
                    = pipeline.CreateSubobject<CD3DX12_HIT_GROUP_SUBOBJECT>();
                if (!closestHit.empty())
                    hitGroup->SetClosestHitShaderImport(closestHit.c_str());
                if (!anyHit.empty())
                    hitGroup->SetAnyHitShaderImport(anyHit.c_str());
            ...
            bindHitGroup(HIT_GROUP_SPHERE_NAME,
                D3D12_HIT_GROUP_TYPE::D3D12_HIT_GROUP_TYPE_TRIANGLES,
                CLOSEST_HIT_NORMAL_NAME);
            bindHitGroup(HIT_GROUP_QUAD_NAME,
                D3D12_HIT_GROUP_TYPE::D3D12_HIT_GROUP_TYPE_TRIANGLES,
                CLOSEST_HIT_NORMAL_NAME);
            bindHitGroup(HIT_GROUP_FLOOR_NAME,
                D3D12_HIT_GROUP_TYPE::D3D12_HIT_GROUP_TYPE_TRIANGLES,
                CLOSEST_HIT_PLANE_NAME);
        }
        {
            CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT* config
                = pipeline.CreateSubobject<
                    CD3DX12_RAYTRACING_SHADER_CONFIG_SUBOBJECT>();
            UINT payloadSize = Framework::Math::MathUtil::mymax<UINT>(
                { sizeof(RayPayload), sizeof(ShadowPayload) });
            UINT attrSize = sizeof(float) * 2;
            config->Config(payloadSize, attrSize);
        }
        {
            auto bindLocalRootSignature = [&pipeline](
            ...
            bindLocalRootSignature(
                mMissLocalRootSignature.Get(), MISS_SHADER_NAME);
            bindLocalRootSignature(mHitGroupLocalRootSignature
                                       [LocalRootSignature::HitGroup::Normal]
                                           .Get(),
        ...
        MY_THROW_IF_FAILED(mDXRDevice->getDXRDevice()->CreateStateObject(
            pipeline, IID_PPV_ARGS(&mDXRStateObject)));
    }
    {
        using namespace Framework::Desc;
        DescriptorTableDesc desc = { L"DescriptorTable", 10000,
            HeapType::CBV_SRV_UAV, HeapFlag::ShaderVisible };
    ...
    auto getOffset = [&mIndexOffsets, &mVertexOffsets](
                         LocalRootSignature::HitGroupIndex::MyEnum e) {
        if (e == 0)
            return std::make_tuple<UINT, UINT>(0, 0);
        else
            return std::make_tuple(std::accumulate(mIndexOffsets.begin(),
                                       mIndexOffsets.begin() + e, 0u),
                std::accumulate(
                    mVertexOffsets.begin(), mVertexOffsets.begin() + e, 0u));
    };

    {
        ...
        std::vector<Index> resourceIndices;
        {
...

            {
                ...
                };
                {
                    auto texture = std::make_shared<Texture2D>(device,
                        createUnitTexture(
                            Color4(1, 1, 1, 1), L"DefaultAlbedo"));
                ...
                }
                {
                    auto texture = std::make_shared<Texture2D>(device,
                        createUnitTexture(
                            Color4(0.5f, 0.5f, 1.0f, 1.0f), L"DefaultNormal"));
                    mDescriptorTable->allocate(texture.get());
                    texture->createSRV(device);
                ...
                }
                {
                    auto texture = std::make_shared<Texture2D>(device,
                        createUnitTexture(
                            Color4(0, 0, 0, 1), L"DefaultMetallicRoughness"));
                    mDescriptorTable->allocate(texture.get());
                    texture->createSRV(device);
                ...
                }
                {
                    auto texture = std::make_shared<Texture2D>(device,
                        createUnitTexture(
                            Color4(0, 0, 0, 1), L"DefaultEmissive"));
                    mDescriptorTable->allocate(texture.get());
                    texture->createSRV(device);
            ...
            auto loadTextures =
                [&](const Material& material,
                    const std::vector<Framework::Desc::TextureDesc>&
                        textureDatas,
                    ...
                    if (textureDatas.empty()) {
                        ...
                        auto texture
                            = std::make_shared<Texture2D>(device, albedo);
                        mDescriptorTable->allocate(texture.get());
                        texture->createSRV(device);
                    ...
                    if (material.normalMapID == -1) {
                        ...
                        auto texture
                            = std::make_shared<Texture2D>(device, normal);
                        mDescriptorTable->allocate(texture.get());
                        texture->createSRV(device);
                        mTextures[nextID] = (texture);
                        mTextureIDs[nextID] = nextID;
                    ...
                    if (material.metalRoughID == -1) {
                        ...
                        auto texture
                            = std::make_shared<Texture2D>(device, metalRough);
                        mDescriptorTable->allocate(texture.get());
                        texture->createSRV(device);
                        mTextures[nextID] = (texture);
                        mTextureIDs[nextID] = nextID;
                    ...
                    if (material.emissiveMapID == -1) {
                        ...
                        auto texture
                            = std::make_shared<Texture2D>(device, emissive);
                        mDescriptorTable->allocate(texture.get());
                        texture->createSRV(device);
                        mTextures[nextID] = (texture);
                        mTextureIDs[nextID] = nextID;
...

            {
                ...
                auto vertices
                    = toLinearVertices(loader.getPositionsPerSubMeshes(),
                        loader.getNormalsPerSubMeshes(),
                        loader.getUVsPerSubMeshes(),
                        loader.getTangentsPerSubMeshes());

                createBuffer(mDeviceResource->getDevice(),
                    &mIndexBuffer[BottomLevelASType::UFO].mResource,
                    indices.data(), indices.size() * sizeof(indices[0]),
                    L"IndexBuffer");
                createBuffer(mDeviceResource->getDevice(),
                    &mVertexBuffer[BottomLevelASType::UFO].mResource,
                    vertices.data(), vertices.size() * sizeof(vertices[0]),
                    L"VertexBuffer");

                resourceIndices.insert(
                    resourceIndices.end(), indices.begin(), indices.end());
                resourceVertices.insert(
                    resourceVertices.end(), vertices.begin(), vertices.end());
                mIndexOffsets[LocalRootSignature::HitGroupIndex::UFO]
                    = (UINT)indices.size();
                mVertexOffsets[LocalRootSignature::HitGroupIndex::UFO]
                    = (UINT)vertices.size();

                auto materialList = loader.getMaterialDatas();
                Material material = {};
                if (!materialList.empty())
                    material = loader.getMaterialDatas()[0];
            ...
            }
            {
                ...
                createBuffer(mDeviceResource->getDevice(),
                    &mIndexBuffer[BottomLevelASType::Quad].mResource,
                    indices.data(), indices.size() * sizeof(indices[0]),
                    L"IndexBuffer");
                ...
                createBuffer(mDeviceResource->getDevice(),
                    &mVertexBuffer[BottomLevelASType::Quad].mResource,
                    vertices.data(), vertices.size() * sizeof(vertices[0]),
                    L"VertexBuffer");
                ...
                resourceVertices.insert(
                    resourceVertices.end(), vertices.begin(), vertices.end());
                mIndexOffsets[LocalRootSignature::HitGroupIndex::Quad]
                    = (UINT)indices.size();
                mVertexOffsets[LocalRootSignature::HitGroupIndex::Quad]
                    = (UINT)vertices.size();

                Framework::Desc::TextureDesc desc
                    = Framework::Utility::TextureLoader::load(
                        texPath / "back2.png");
                auto texture = std::make_shared<Texture2D>(device, desc);
                mDescriptorTable->allocate(texture.get());
                texture->createSRV(device);
                mTextures[ModelTextureType::Quad_Albedo] = (texture);
                mTextureIDs[ModelTextureType::Quad_Albedo]
                    = ModelTextureType::Quad_Albedo;
            }

            {
                ...
                auto vertices
                    = toLinearVertices(loader.getPositionsPerSubMeshes(),
                        ...
                        loader.getUVsPerSubMeshes());
                createBuffer(mDeviceResource->getDevice(),
                    &mIndexBuffer[BottomLevelASType::Floor].mResource,
                    indices.data(), indices.size() * sizeof(indices[0]),
                    L"IndexBuffer");
                createBuffer(mDeviceResource->getDevice(),
                    &mVertexBuffer[BottomLevelASType::Floor].mResource,
                    vertices.data(), vertices.size() * sizeof(vertices[0]),
                    L"VertexBuffer");
                ...
                resourceVertices.insert(
                    resourceVertices.end(), vertices.begin(), vertices.end());
                mIndexOffsets[LocalRootSignature::HitGroupIndex::Floor]
                    = (UINT)indices.size();
                mVertexOffsets[LocalRootSignature::HitGroupIndex::Floor]
                    = (UINT)vertices.size();
        ...
        }
        {
            ...
            ID3D12GraphicsCommandList* commandList
                = mDeviceResource->getCommandList();
            ID3D12CommandAllocator* allocator
                = mDeviceResource->getCommandAllocator();
            ...
            for (int i = 0; i < BottomLevelASType::Count; i++) {
                ...
                geometryDesc.Type = D3D12_RAYTRACING_GEOMETRY_TYPE::
                    D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;
                geometryDesc.Triangles.IndexBuffer
                    = mIndexBuffer[i].mResource->GetGPUVirtualAddress();
                geometryDesc.Triangles.IndexCount
                    = static_cast<UINT>(
                          mIndexBuffer[i].mResource->GetDesc().Width)
                    / sizeof(Index);
                ...
                geometryDesc.Triangles.VertexCount
                    = static_cast<UINT>(
                          mVertexBuffer[i].mResource->GetDesc().Width)
                    / sizeof(Framework::DX::Vertex);
                geometryDesc.Triangles.VertexFormat
                    = DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT;
                geometryDesc.Flags = D3D12_RAYTRACING_GEOMETRY_FLAGS::
                ...
                bottomLevelInputs.DescsLayout
                    = D3D12_ELEMENTS_LAYOUT::D3D12_ELEMENTS_LAYOUT_ARRAY;
                ...
                MY_THROW_IF_FALSE(
                    bottomLevelPreInfo.ResultDataMaxSizeInBytes > 0);

                allocateUAVBuffer(device,
                    bottomLevelPreInfo.ScratchDataSizeInBytes,
                ...
                allocateUAVBuffer(device,
                    bottomLevelPreInfo.ResultDataMaxSizeInBytes,
                    &mBLASBuffers[i].buffer, initResourceState,
                    L"BottomLevelAS");
            ...
            D3D12_RESOURCE_STATES initResourceState = D3D12_RESOURCE_STATES::
                D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE;
            allocateUAVBuffer(device, topLevelPreInfo.ResultDataMaxSizeInBytes,
                &mTLASBuffer.buffer, initResourceState, L"TopLevelAS");
            ...
            createBuffer(device, &mResourcesIndexBuffer.mResource,
                resourceIndices.data(),
                resourceIndices.size() * sizeof(resourceIndices[0]),
                L"ResourceIndex");
            createBuffer(device, &mResourcesVertexBuffer.mResource,
                resourceVertices.data(),
                resourceVertices.size() * sizeof(resourceVertices[0]),
                L"ResourceVertex");

            {
                UINT indexBufferSize = (UINT)resourceIndices.size()
                    * sizeof(resourceIndices[0]) / sizeof(float);
                D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
                srvDesc.ViewDimension
                    = D3D12_SRV_DIMENSION::D3D12_SRV_DIMENSION_BUFFER;
                ...
                device->CreateShaderResourceView(
                    mResourcesIndexBuffer.mResource.Get(), &srvDesc,
            ...
            }
            {
                ...
                srvDesc.Buffer.StructureByteStride
                    = sizeof(resourceVertices[0]);
                mDescriptorTable->allocate(&mResourcesVertexBuffer);
                device->CreateShaderResourceView(
                    mResourcesVertexBuffer.mResource.Get(), &srvDesc,
    ...
    }
    {
        ...
        void* rayGenShaderID
            = stateObjectProp->GetShaderIdentifier(RAY_GEN_NAME.c_str());
        void* missShaderID
            = stateObjectProp->GetShaderIdentifier(MISS_SHADER_NAME.c_str());
        void* missShadowShaderID = stateObjectProp->GetShaderIdentifier(
            MISS_SHADOW_SHADER_NAME.c_str());
        void* hitGroup_SphereShaderID = stateObjectProp->GetShaderIdentifier(
            HIT_GROUP_SPHERE_NAME.c_str());
        void* hitGroup_QuadShaderID
            = stateObjectProp->GetShaderIdentifier(HIT_GROUP_QUAD_NAME.c_str());
        void* hitGroup_FloorShaderID = stateObjectProp->GetShaderIdentifier(
            HIT_GROUP_FLOOR_NAME.c_str());
        ...
        }
        {
            ...
            {
                rootArguments.cb.indexOffset = std::get<0>(
                    getOffset(LocalRootSignature::HitGroupIndex::UFO));
                rootArguments.cb.vertexOffset = std::get<1>(
                    getOffset(LocalRootSignature::HitGroupIndex::UFO));
                rootArguments.albedo
                    = mTextures[mTextureIDs[ModelTextureType::UFO_Albedo]]
                          ->getGPUHandle();
                rootArguments.normal
                    = mTextures[mTextureIDs[ModelTextureType::UFO_Normal]]
                          ->getGPUHandle();
                rootArguments.metalRough
                    = mTextures[mTextureIDs[ModelTextureType::UFO_MetalRough]]
                          ->getGPUHandle();
                rootArguments.emissive
                    = mTextures[mTextureIDs[ModelTextureType::UFO_Emissive]]
                          ->getGPUHandle();
                table.push_back(ShaderRecord(hitGroup_SphereShaderID,
                    shaderIDSize, &rootArguments, sizeof(RootArgument)));
            }
            {
                rootArguments.cb.indexOffset = std::get<0>(
                    getOffset(LocalRootSignature::HitGroupIndex::Quad));
                rootArguments.cb.vertexOffset = std::get<1>(
                    getOffset(LocalRootSignature::HitGroupIndex::Quad));
                rootArguments.albedo
                    = mTextures[mTextureIDs[ModelTextureType::Quad_Albedo]]
                          ->getGPUHandle();
                table.push_back(ShaderRecord(hitGroup_QuadShaderID,
                    shaderIDSize, &rootArguments, sizeof(RootArgument)));
            }
            {
                rootArguments.cb.indexOffset = std::get<0>(
                    getOffset(LocalRootSignature::HitGroupIndex::Floor));
                rootArguments.cb.vertexOffset = std::get<1>(
                    getOffset(LocalRootSignature::HitGroupIndex::Floor));
                rootArguments.albedo
                    = mTextures[mTextureIDs[ModelTextureType::Plane_Albedo]]
                          ->getGPUHandle();
                rootArguments.normal
                    = mTextures[mTextureIDs[ModelTextureType::Plane_Normal]]
                          ->getGPUHandle();
                rootArguments.metalRough
                    = mTextures[mTextureIDs[ModelTextureType::Plane_MetalRough]]
                          ->getGPUHandle();
                rootArguments.emissive
                    = mTextures[mTextureIDs[ModelTextureType::Plane_Emissive]]
                          ->getGPUHandle();
                table.push_back(ShaderRecord(hitGroup_FloorShaderID,
                    shaderIDSize, &rootArguments, sizeof(RootArgument)));
            }
...
void Scene::createWindowDependentResources() {
    ...
    auto heapProps
        = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE::D3D12_HEAP_TYPE_DEFAULT);
    MY_THROW_IF_FAILED(device->CreateCommittedResource(&heapProps,
        D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &uavResourceDesc,
        D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
    ...
    device->CreateUnorderedAccessView(mRaytracingOutput.mResource.Get(),
        nullptr, &uavDesc, mRaytracingOutput.mCPUHandle);
}
